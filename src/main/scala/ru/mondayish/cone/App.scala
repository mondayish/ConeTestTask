...
object App {
  ...
  private val result: ListBuffer[List[Square]] = ListBuffer()
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(result.map(x => makeListOfSquaresPretty(x)).mkString("\n"))
  }

  def findAllPermutations(start: List[Square], nums: List[Square]): Unit = {
    val size = nums.size
    if (size == 0 && checkPermutation(start)) {
      ...
      for (i <- 0 until size) {
        ...
        if (isGreenPositionsOk(permutations) && isYellowPositionsOk(permutations) &&
          isGreenFillCorrectly(permutations) && isYellowFillCorrectly(permutations)) {
  ...
  def checkPermutation(permutation: List[Square]): Boolean =
    isGreenPositionsOk(permutation) && isYellowPositionsOk(permutation)
  ...
  def makeListOfSquaresPretty(permutations: List[Square]): String = {
    permutations.map(square => square.toString).mkString("\n")
  }
}
